...
export function JoinServerModal({ open, onClose, initialCode }: JoinServerModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Join with a Code" size="sm">
      <form onSubmit={handleJoin} className="space-y-4">
        <p className="text-pulse-text-muted text-sm">
          Enter a 6-digit invite code to join a private server.{' '}
          <button type="button" onClick={browseDiscover} className="text-pulse-brand hover:underline">
            Browse public communities instead
          </button>
        </p>
